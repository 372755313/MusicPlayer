...
@property (nonatomic, copy) NSArray *otherButtondTitles;
@property (nonatomic, copy) ZYActionSheetViewDidSelectButtonBlock selectRowBlock;
                      ...
                      handler:(ZYActionSheetViewDidSelectButtonBlock)block;
{
    ...
    if (self)
    {
        ...
        _otherButtondTitles = otherButtondTitles;
        ...
        UIImage *normZYImage = [self imageWithColor:[UIColor whiteColor]];
        ...
        if (_title && _title.length>0)
        {
            ...
            titleLabel.textColor =  HWColor(98, 100, 101);
        ...
        if ([_otherButtonTitles count] > 0)
        {
            for (int i = 0; i < _otherButtonTitles.count; i++)
            {
                ...
                btn.frame = CGRectMake(0, offy, width, 1.5*kRowHeight);
                btn.tag = i;
                btn.backgroundColor = [UIColor whiteColor];
//                btn.titleLabel.font = [UIFont systemFontOfSize:kButtonTitleFontSize];
//                [btn setTitleColor:HWColor(98, 100, 101) forState:UIControlStateNormal];
//                [btn setTitle:_otherButtonTitles[i] forState:UIControlStateNormal];
                [btn setBackgroundImage:normZYImage forState:UIControlStateNormal];
                ...
                UILabel *titleL = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, width, 1.0*kRowHeight)];
                titleL.textAlignment = NSTextAlignmentCenter;
                titleL.font = [UIFont systemFontOfSize:kButtonTitleFontSize];
                titleL.textColor =HWColor(98, 100, 101);
                titleL.text = _otherButtonTitles[i];
                [btn addSubview:titleL];
                
                UILabel *dtitleL = [[UILabel alloc]initWithFrame:CGRectMake(0, 0.8*kRowHeight, kWidth, 0.6*kRowHeight)];
                dtitleL.textAlignment = NSTextAlignmentCenter;
                dtitleL.font = [UIFont systemFontOfSize:15];
                dtitleL.textColor =HWColor(183, 183, 183);
                dtitleL.text = _otherButtondTitles[i];
                [btn addSubview:dtitleL];
                
                
                [_actionSheetView addSubview:btn];
                offy += 1.5*kRowHeight+kRowLineHeight;
            }
        ...
        if (_destructiveButtonTitle && _destructiveButtonTitle.length>0)
        {
            ...
            [destructiveButton setBackgroundImage:normZYImage forState:UIControlStateNormal];
        ...
        if (cancelButtonTitle != nil) {
            ...
            [cancelBtn setTitleColor:HWColor(98, 100, 101) forState:UIControlStateNormal];
            [cancelBtn setTitle:_cancelButtonTitle ?: @"取消" forState:UIControlStateNormal];
            [cancelBtn setBackgroundImage:normZYImage forState:UIControlStateNormal];
...
+ (ZYActionSheetView *)showActionSheetWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles otherButtondTitles:(NSArray *)otherButtondTitles handler:(ZYActionSheetViewDidSelectButtonBlock)block
{
    ZYActionSheetView *actionSheetView  = [[ZYActionSheetView alloc]initWithTitle:title cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitles otherButtondTitles:otherButtondTitles handler:block];
