...
NSString *const HWAlertViewWillShowNotification = @"HWAlertViewWillShowNotification";
NSString *const HWAlertViewDidShowNotification = @"HWAlertViewDidShowNotification";
NSString *const HWAlertViewWillDismissNotification = @"HWAlertViewWillDismissNotification";
NSString *const HWAlertViewDidDismissNotification = @"HWAlertViewDidDismissNotification";
...
const UIWindowLevel UIWindowLevelHWAlert = 1996.0;  // don't overlap system's alert
const UIWindowLevel UIWindowLevelHWAlertBackground = 1985.0; // below the alert window

@class HWAlertBackgroundWindow;

static NSMutableArray *__si_alert_queue;
static BOOL __si_alert_animating;
static HWAlertBackgroundWindow *__si_alert_background_window;
static HWAlertView *__si_alert_current_view;

@interface HWAlertView ()<CAAnimationDelegate>
...
@interface HWAlertBackgroundWindow : UIWindow

@end

@interface HWAlertBackgroundWindow ()

@property (nonatomic, assign) HWAlertViewBackgroundStyle style;

@end

@implementation HWAlertBackgroundWindow

- (id)initWithFrame:(CGRect)frame andStyle:(HWAlertViewBackgroundStyle)style
{
    ...
    if (self) {
        ...
        self.opaque = NO;
        self.windowLevel = UIWindowLevelHWAlertBackground;
    }
    return self;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    switch (self.style) {
        case HWAlertViewBackgroundStyleGradient:
        {
        ...
        case HWAlertViewBackgroundStyleSolid:
        {
...
#pragma mark - HWAlertItem

@interface HWAlertItem : NSObject

@property (nonatomic, copy) NSString *title;
@property (nonatomic, assign) HWAlertViewButtonType type;
@property (nonatomic, copy) HWAlertViewHandler action;

@end

@implementation HWAlertItem

@end

#pragma mark - HWAlertViewController
...
+ (void)initialize
{
    if (self != [HWAlertView class])
        return;
    
    HWAlertView *appearance = [self appearance];
...
+ (void)setCurrentAlertView:(HWAlertView *)alertView
{
...
+ (void)showBackground
{
    if (!__si_alert_background_window) {
        ...
        __si_alert_background_window = [[HWAlertBackgroundWindow alloc] initWithFrame:frame
                                                                             andStyle:[HWAlertView currentAlertView].backgroundStyle];
...
{
    HWAlertItem *item = [[HWAlertItem alloc] init];
	item.title = title;
	item.type = type;
	item.action = handler;
...
- (void)show
{
    ...
    if (![[HWAlertView sharedQueue] containsObject:self]) {
        [[HWAlertView sharedQueue] addObject:self];
    }
    
    if ([HWAlertView isAnimating]) {
        return; // wait for next turn
    }
    
    if ([HWAlertView currentAlertView].isVisible) {
        HWAlertView *alert = [HWAlertView currentAlertView];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:HWAlertViewWillShowNotification object:self userInfo:nil];
    
    self.visible = YES;
    
    [HWAlertView setAnimating:YES];
    [HWAlertView setCurrentAlertView:self];
    
    // transition background
    [HWAlertView showBackground];
    
    HWAlertViewController *viewController = [[HWAlertViewController alloc] initWithNibName:nil bundle:nil];
    ...
    if (!self.alertWindow) {
        ...
        window.opaque = NO;
        window.windowLevel = UIWindowLevelHWAlert;
    ...
    [self transitionInCompletion:^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HWAlertViewDidShowNotification object:self userInfo:nil];
        #ifdef __IPHONE_7_0
        [self addParallaxEffect];
        #endif
        
        [HWAlertView setAnimating:NO];
        
        NSInteger index = [[HWAlertView sharedQueue] indexOfObject:self];
        if (index < [HWAlertView sharedQueue].count - 1) {
...
- (void)dismissAnimated:(BOOL)animated cleanup:(BOOL)cleanup
{
    ...
    if (isVisible) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HWAlertViewWillDismissNotification object:self userInfo:nil];
        #ifdef __IPHONE_7_0
    ...
    void (^dismissComplete)(void) = ^{
        ...
        [HWAlertView setCurrentAlertView:nil];
        
        HWAlertView *nextAlertView;
        NSInteger index = [[HWAlertView sharedQueue] indexOfObject:self];
        if (index != NSNotFound && index < [HWAlertView sharedQueue].count - 1) {
            nextAlertView = [HWAlertView sharedQueue][index + 1];
        }
        
        if (cleanup) {
            [[HWAlertView sharedQueue] removeObject:self];
        }
        ...
        if (isVisible) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:HWAlertViewDidDismissNotification object:self userInfo:nil];
        }
        ...
        if (nextAlertView) {
            ...
            if ([HWAlertView sharedQueue].count > 0) {
                HWAlertView *alert = [[HWAlertView sharedQueue] lastObject];
                [alert show];
            }
        }
    };
    ...
    if (animated && isVisible) {
        ...
        if ([HWAlertView sharedQueue].count == 1) {
            [HWAlertView hideBackgroundAnimated:YES];
        }
...
- (void)transitionInCompletion:(void(^)(void))completion
{
    switch (self.transitionStyle) {
        case HWAlertViewTransitionStyleSlideFromBottom:
        {
            ...
            break;
        case HWAlertViewTransitionStyleSlideFromTop:
        {
            CGRect rect = self.containerView.frame;
            CGRect originalRect = rect;
            ...
            break;
        case HWAlertViewTransitionStyleFade:
        {
            ...
            break;
        case HWAlertViewTransitionStyleBounce:
        {
            ...
            break;
        case HWAlertViewTransitionStyleDropDown:
        {
...
- (void)validateLayout
{
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2 && self.buttonsListStyle == HWAlertViewButtonsListStyleNormal) {
            ...
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                ...
                if (self.buttons.count > 1) {
                    if (i == self.buttons.count - 1 && ((HWAlertItem *)self.items[i]).type == HWAlertViewButtonTypeCancel) {
...
- (CGFloat)preferredHeight
{
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count <= 2 && self.buttonsListStyle == HWAlertViewButtonsListStyleNormal) {
            ...
            if (self.buttons.count > 2 && ((HWAlertItem *)[self.items lastObject]).type == HWAlertViewButtonTypeCancel) {
...
{
    HWAlertItem *item = self.items[index];
	...
	switch (item.type) {
		case HWAlertViewButtonTypeCancel:
			normalImage = [UIImage imageNamed:@"HWAlertView.bundle/button-cancel"];
			highlightedImage = [UIImage imageNamed:@"HWAlertView.bundle/button-cancel-d"];
			...
			break;
		case HWAlertViewButtonTypeDestructive:
			normalImage = [UIImage imageNamed:@"HWAlertView.bundle/button-destructive"];
			highlightedImage = [UIImage imageNamed:@"HWAlertView.bundle/button-destructive-d"];
			...
			break;
		case HWAlertViewButtonTypeDefault:
		default:
			normalImage = [UIImage imageNamed:@"HWAlertView.bundle/button-default"];
			highlightedImage = [UIImage imageNamed:@"HWAlertView.bundle/button-default-d"];
...
{
	[HWAlertView setAnimating:YES]; // set this flag to YES in order to prevent showing another alert in action block
    HWAlertItem *item = self.items[button.tag];
	if (item.action) {
		item.action(self);
	}
...
- (void)setButtonColor:(UIColor *)buttonColor
{
    ...
    [self setColor:buttonColor toButtonsOfType:HWAlertViewButtonTypeDefault];
}
...
- (void)setCancelButtonColor:(UIColor *)buttonColor
{
    ...
    [self setColor:buttonColor toButtonsOfType:HWAlertViewButtonTypeCancel];
}
...
- (void)setDestructiveButtonColor:(UIColor *)buttonColor
{
    ...
    [self setColor:buttonColor toButtonsOfType:HWAlertViewButtonTypeDestructive];
}


- (void)setDefaultButtonImage:(UIImage *)defaultButtonImage forState:(UIControlState)state
{
    [self setButtonImage:defaultButtonImage forState:state andButtonType:HWAlertViewButtonTypeDefault];
}


- (void)setCancelButtonImage:(UIImage *)cancelButtonImage forState:(UIControlState)state
{
    [self setButtonImage:cancelButtonImage forState:state andButtonType:HWAlertViewButtonTypeCancel];
}


- (void)setDestructiveButtonImage:(UIImage *)destructiveButtonImage forState:(UIControlState)state
{
    [self setButtonImage:destructiveButtonImage forState:state andButtonType:HWAlertViewButtonTypeDestructive];
}


- (void)setButtonImage:(UIImage *)image forState:(UIControlState)state andButtonType:(HWAlertViewButtonType)type
{
    for (NSUInteger i = 0; i < self.items.count; i++)
    {
        HWAlertItem *item = self.items[i];
        if(item.type == type)
        {
            UIButton *button = self.buttons[i];
...
-(void)setColor:(UIColor *)color toButtonsOfType:(HWAlertViewButtonType)type {
    for (NSUInteger i = 0; i < self.items.count; i++) {
        HWAlertItem *item = self.items[i];
        if(item.type == type) {
            UIButton *button = self.buttons[i];
